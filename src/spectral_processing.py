...
def wavelength_to_rgb(wavelength, gamma=0.8):
    ...
    """
    if len(np.hstack([wavelength])) > 1:
        wavelength = np.average(wavelength)
...
def spectral_weights(wl, spectrum):
    ...
    spectrum_PDF = q_data / np.trapz(q_data, wl_data)
    ...
    weights_wl = np.interp(wl, wl_data, spectrum_PDF, left=0.0, right=0.0)
    return weights_wl


def solar_weighted_absorptance(
    wl,
    abso,
    solar_spectrum="/media/ael/Flashy/backup_05-06-2021/Documents/Boulot/Material_properties/astmg173.txt",
):
    """
...
def solar_weighted_reflectance(
    wl,
    ref,
    solar_spectrum="/media/ael/Flashy/backup_05-06-2021/Documents/Boulot/Material_properties/astmg173.txt",
):
...
def planck_weights(wl, T):
    ...
    norm = 5.67e-8 * (T**4.0)  # Stefan-Boltzmann law
    weights = (
        (2.0 * np.pi * h * c**2.0)
        / ((wl) ** 5.0)
        / (np.exp(h * c / (wl * k * T)) - 1.0)
        / norm
    )
    return weights
...
def co_interpolate_spectra(wl1, data1, wl2, data2):
    ...
    wl1, data1 = np.sort(wl1), data1[np.argsort(wl1)]
    wl2, data2 = np.sort(wl2), data2[np.argsort(wl2)]
    # define interpolation range:
    xmin, xmax = np.amin(np.hstack([wl1, wl2])), np.amax(np.hstack([wl1, wl2]))
    wl_inter = np.linspace(xmin, xmax, np.amax([len(wl1), len(wl2)]))
    # Interpolate spectra
    data1_inter = np.interp(wl_inter, wl1, data1)
    data2_inter = np.interp(wl_inter, wl2, data2)

    return wl_inter, data1_inter, data2_inter


def diff_spectra(wl1, data1, wl2, data2):
    wl_inter, data1_inter, data2_inter = co_interpolate_spectra(
        wl1, data1, wl2, data2
    )
    """
...
def process_spectrum(wl, data, clean=None, smooth=None):
    ...
    if clean is not None:
        ...
        data_r = np.copy(data)
        for c in clean:
            remove = np.logical_and(wl_r >= c[0], wl_r <= c[1])
            wl_r = wl_r[~remove]
            data_r = data_r[~remove]
        data = np.interp(wl, wl_r, data_r)

    if smooth is not None:
        for s in smooth:
            kernel = np.ones(s[2]) / s[2]
            interval = np.logical_and(wl >= s[0], wl <= s[1])
            datai = np.convolve(data[interval], kernel, mode="same")
