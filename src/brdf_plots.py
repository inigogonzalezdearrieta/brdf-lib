...
def plot_inplane_BDRF(
    ...
    **kwargs,
    ...
    theta_i_deg = (
        -theta_i_deg
    )  # we choose to ploit the incident angle in the negative values.
    thetas, theta_i = thetas_r_deg * rad, theta_i_deg * rad

    if diffuse_ratio:
        dar = np.trapz(
            bdrfs * np.cos(thetas), thetas
        )  # directional-arcuate reflectance
        brdf_diff = (
            dar / 2.0
        )  # integration over [-pi2, pi/2] of cos(t)dt, the arc, leads to this
    ...
    bdrf_int = interp1d(
        thetas, bdrfs
    )  # makes the plot look better by interpolating lineraly between angles. Otherwise, the plot will make a straight line in polar coordinates, which is wrong.
    thetas_plot = np.linspace(
        np.amin(thetas), np.amax(thetas), 10 * len(thetas) + 1
    )
...
def plot_BDRF(
    ...
    points = np.array([th_rs, np.sin(th_rs) * phi_rs]).T
    ...
    ths, phs = (
        np.linspace(0.0, thmax_deg, 200) * rad,
        np.linspace(0.0, 360.0, 400) * rad,
    )
    TH, PHI = np.meshgrid(ths, phs)
    # Original interpolator
    BDRF = interpolator(TH, PHI * np.sin(TH))
    ...
    if specularity_indicators:
        ...
        plt.scatter(
            np.pi + phi_i_deg * rad, th_i_deg, marker="+", color="r", s=50
        )
    plt.savefig(saveloc)
    plt.close()
