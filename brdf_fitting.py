...
def build_inc_interpolator(thetas_r, phis_r, bdrf):
    ...
    """
    points = np.array([thetas_r, phis_r]).T
...
def clean_data(thetas, phis, data, K=6, deshade=True):
    ...
    for i, d in enumerate(data):
        if nans[i] or deshade:
            psis = np.arccos(
                np.sin(thetas[i]) * sin_t
                + np.cos(thetas[i]) * cos_t * np.cos(phis[i] - phis)
            )
            good_psis = psis[~nans]
            # Find non-nan K nearest points
            closest = np.argsort(good_psis)[1 : K + 1]
            weights = 1.0 / good_psis[closest]
            # If d is NaN, we find the K nearest neighbours that are not NaNs and interpolate
            if nans[i]:
                # Interpo;late (weighted average):
                data[i] = np.sum(weights * data[~nans][closest]) / np.sum(
                    weights
                )
            elif deshade:
                ...
                if np.sum(below) > (K - 2):
                    ...
                    data[i] = np.sum(weights * good_data[below]) / np.sum(
                        weights
                    )
...
def get_incidence_plane_bdrf(
    th_i, phi_i, thetas_r, phis_r, bdrf, th_rs, deshade=True
):
    """
    This is only valid for a single incidence direction and a single wavelength or spectrally integrated bdrf.
    ...
    phi_r = np.ones(len(th_rs)) * phi_i
    in_plane_bdrf = np.zeros(len(th_rs))

    neg = th_rs < 0.0
    phi_r[~neg] = (phi_i + np.pi) % (
        2.0 * np.pi
    )  # positive thetas are the forward scattered directions, negative ones are the backward scattered direction

    in_plane_bdrf[neg] = interpolator(-th_rs[neg], phi_r[neg])
    in_plane_bdrf[neg] = clean_data(
        -th_rs[neg], phi_r[neg], in_plane_bdrf[neg], deshade=True
    )
    in_plane_bdrf[~neg] = interpolator(th_rs[~neg], phi_r[~neg])
    in_plane_bdrf[~neg] = clean_data(
        th_rs[~neg], phi_r[~neg], in_plane_bdrf[~neg], deshade=True
    ...
    in_plane_bdrf[th_rs == 0] = (
        in_plane_bdrf[neg][-1] + in_plane_bdrf[~neg][1]
    ) / 2.0
...
def CT_wrap(inc_angle_rad, det_angle_rad, m, R, alpha):
    ...
    """
    d_i = -np.array([np.sin(inc_angle_rad), 0.0, np.cos(inc_angle_rad)])
    d_n = np.array([0.0, 0.0, 1.0])
    d_r = np.array([np.sin(det_angle_rad), 0, np.cos(det_angle_rad)])
...
def CT_fit(inc_angle_rad, det_angle_rad, ref_tot):
    ...
    var0 = np.array(
        [1.1, 0.0001, 0.4]
    )  # Initial conditions: m, R_dh_Lam, alpha
    fixed_var = [inc_angle_rad, det_angle_rad, ref_tot]
    # Optimisation bounds
    bounds = np.array([[1.0, 1.5], [0.0, np.amin(ref_tot)], [0.0, 5.0]])

    # Optimisation constraints
    constraints = [
        {"type": "ineq", "fun": lambda x: x - bounds[:, 0]},
        {"type": "ineq", "fun": lambda x: bounds[:, 1] - x},
    ]
    # Results
    res = minimize(
        least_square_diff,
    ...
    return m, R_dh_Lam, alpha, np.array(CT)
...
def CT_curve_fit(inc_angle_rad, det_angle_rad, ref_tot, daq=None):
    ...
    fixed_var = [inc_angle_rad, det_angle_rad, ref_tot]
    # Optimisation bounds
    bounds = (np.array([1.0, 0.0, 0.1]), np.array([1.1, 0.002, 2.0]))
    ...
    if daq is not None:
        ...
        s_daq = (
            0.02 * daq + s_daq0
        )  # this is mainly a systematic error on the reference beam measurement
        ...
        BRDF = daq / np.cos(det_angle_rad) / phi_0
        # considering both s_daq and s_th_r
        deg = np.pi / 180.0
        s_th_r = (
            0.3 * deg
        )  # uncercainty on th_r (mainly due to spot size + sensor size)
        u = np.sqrt(daq**2 * s_th_r**2 * np.tan(det_angle_rad) ** 2 + s_daq**2)
        d = np.cos(det_angle_rad) * phi_0
        s_BRDF = u / d
        # Results
        popt, pcov = curve_fit(
            f=CT_param,
        ...
        s_BDRF = 1 / np.cos(det_angle_rad) / 10.0
        popt, pcov = curve_fit(
            f=CT_param,
            xdata=det_angle_rad,
            ydata=ref_tot,
...
def least_square_diff(var, fixed_var):
    ...
    for i, d in enumerate(det_angles):
        CT.append(CT_wrap(inc_angle, d, m, R_dh_Lam, alpha))
    norm_bdrf = np.amax(bdrfs_mes)
    lsd = np.trapz((np.array(bdrfs_mes) - np.array(CT)) ** 2)
    return lsd
